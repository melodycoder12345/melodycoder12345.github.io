...
(function () {
  ...
  function isDetailPage() {
    ...
    return /\/(algo|db|kafka|redis|linux|network|cs|golang|distributed|ai)\/(?!index\.html$)[^/]+\.html$/.test(path);
  }

  if (isDetailPage()) return;

  function friendLinksHref() {
    var inModule = /\/(algo|db|kafka|redis|linux|network|cs|golang|distributed|ai)\//.test(window.location.pathname);
